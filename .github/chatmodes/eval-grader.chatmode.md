...
description: >
  ...
  Used only inside .orchestrator/scripts/run-agent-evals.sh, never against real features
...
3. **The rubric** (`.orchestrator/evals/<agent>/rubric.yml`) — the full list of criteria for this agent, each tagged with which case IDs it applies to.
...
- **Do not average criteria into a single score and report only that.** A rubric with 5 criteria and 1 failure is not "80% — mostly fine." Report every criterion's individual verdict; let whoever reads the result (a human, or the regression-gate logic in `.orchestrator/scripts/run-agent-evals.sh`) decide what threshold matters.
- **Do not modify, "fix," or regenerate the target agent's output.** Your only job is to score what you were given.

## Required output format

Write one JSON object per case graded, appended to `.orchestrator/evals/results/<agent>-<date>.json` by `.orchestrator/scripts/run-agent-evals.sh` (you produce the object; the calling script handles the file append):
