...
#
# Usage:
#   ./.orchestrator/scripts/update-ticket-stage.sh <issue-number> --field "Stage" --value "Architecture Review"
#   ./.orchestrator/scripts/update-ticket-stage.sh <issue-number> --field "Current Agent" --value "architect"
#   ./.orchestrator/scripts/update-ticket-stage.sh <issue-number> --field "Feature Slug" --value "user-avatar-upload"
#
...
echo "==> Resolving project node ID for project #${PROJECT_NUMBER}..."
# `gh project item-edit --project-id` requires the project's GraphQL node ID (e.g. "PVT_..."),
# NOT the human-facing project number — a mismatch here fails silently-ish with a confusing
# GraphQL error, so resolve it explicitly rather than reusing PROJECT_NUMBER directly.
PROJECT_ID=$(gh project view "${PROJECT_NUMBER}" --owner "${PROJECT_OWNER}" --format json | jq -r '.id')
if [[ -z "${PROJECT_ID}" || "${PROJECT_ID}" == "null" ]]; then
  echo "ERROR: could not resolve node ID for project #${PROJECT_NUMBER} (owner ${PROJECT_OWNER})." >&2
  exit 1
fi
